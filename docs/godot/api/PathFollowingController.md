...
3. **Head-on検出**: 0.3秒待機後、相手も待機中（対面衝突）なら側方回避に切り替え

**側方回避（Sidestep）:**
- 両者が互いに待機している場合（head-on collision）に発動
- 壁をレイキャストでチェックし、安全な方向へ0.5秒間側方移動
- 優先度に基づき左右どちらに避けるか決定（偶数優先度→右、奇数→左）
- 狭い廊下でも片方が右、片方が左に避けることですれ違い可能
