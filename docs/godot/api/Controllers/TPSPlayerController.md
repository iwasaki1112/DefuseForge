...
**実行順序:**
1. **近接攻撃チェック**（最優先 — 1.5m以内の敵を自動検出、一撃キル）
2. CombatAwareness処理（敵検知）
3. エイム方向更新
4. Gun Down検出（壁/味方接近時に武器を下げる、100ms間隔）
5. 射撃判定（gun_down中はブロック）
6. 移動+アニメーション更新
7. カメラ追従

> **近接攻撃（メレー）**: 敵がMELEE_RANGE（1.5m）以内に入ると自動発火。円形360°検出、敵方向に自動回転、`game_rifle_hard`アニメーション再生、100%一撃キル。スプリント中・他アクション中はブロック。
...
### set_sprinting(value: bool) -> void
スプリント状態を設定する。スプリントボタンの押下/解放時にGameScreenから呼ばれる。

**スプリント中の挙動:**
- 右スティックエイム無効化
- 自動射撃無効化
- キャラクターは移動方向を向く
- 移動速度がSPRINT_SPEED（6.0m/s）になる

PC: Shiftキーでもスプリント可能（ボタン不要）。
