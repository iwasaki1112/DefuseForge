...
2種類のグレネード（ハンドグレネード / スモークグレネード）それぞれにボタンがあり、押下でエイミングモードに入ります。

- **ハンドグレネード**: 着地即爆発、距離減衰ダメージ。1ラウンド`HAND_GRENADE_PER_ROUND`個
- **スモークグレネード**: 着地即展開、視界遮断スモーク。1ラウンド`SMOKE_GRENADE_PER_ROUND`個
- **エイミング共通**: レンジインジケーター表示、ボタンオレンジハイライト
- **投擲分岐**: `_is_hand_grenade_aiming`フラグで投擲時にハンドグレネード/スモークグレネードを自動分岐
