...
| 継承元 | `TextureButton` |
| クラス名 | `AnimatedButton` |
| ファイルパス | `scripts/ui/animated_button.gd` |

## 概要

TextureButtonを継承し、押下時に拡大→縮小アニメーションを自動的に再生する。ピボットは自動的に中央に設定される。
...
```gdscript
# シーンでAnimatedButtonを使用（自動アニメーション）
# TextureButtonの代わりにAnimatedButtonを継承するだけ

# コードからアニメーション再生
button.trigger_animation()
```

## 関連クラス

- [AnimatedTextButton](AnimatedTextButton.md) - Button版
