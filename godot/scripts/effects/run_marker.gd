...
var _run_marker_type: RunMarkerType = RunMarkerType.START
...
func _build_icon() -> void:
	if _run_marker_type == RunMarkerType.START:
		_build_play_icon()
	else:
		_build_stop_icon()
...
## マーカーを配置してタイプを設定
## @param pos: パス上の位置
## @param type: RunMarkerType.START または RunMarkerType.END
func set_position_and_type(pos: Vector3, type: RunMarkerType) -> void:
	# 位置を設定
	global_position = Vector3(pos.x, height_offset, pos.z)

	# タイプが変わったらメッシュを再構築
	if _run_marker_type != type:
		_run_marker_type = type
		_update_circle_color()
		_build_mesh()


## タイプを取得
func get_run_marker_type() -> RunMarkerType:
	return _run_marker_type
