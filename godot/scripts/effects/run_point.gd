...
## Runポイント（開始点 = 三角再生アイコン、終点 = 四角停止アイコン）
## パス上のRun区間の開始/終了位置に表示される

enum RunPointType { START, END }
...
var _run_point_type: RunPointType = RunPointType.START


func _init() -> void:
	# デフォルト色を設定
	circle_color = Color(1.0, 0.5, 0.0, 0.95)
	icon_color = Color(1.0, 1.0, 1.0, 1.0)


func get_action_point_type() -> PointType:
	return PointType.RUN
...
## アイコンを構築（タイプに応じて三角または四角）
func _build_icon() -> void:
	if _run_point_type == RunPointType.START:
		_build_play_icon()
	else:
		_build_stop_icon()
...
## ポイントを配置してタイプを設定
## @param pos: パス上の位置
## @param type: RunPointType.START または RunPointType.END
func set_position_and_type(pos: Vector3, type: RunPointType) -> void:
	# 位置を設定
	global_position = Vector3(pos.x, height_offset, pos.z)

	# タイプが変わったらメッシュを再構築
	if _run_point_type != type:
		_run_point_type = type
		_update_circle_color()
		_build_mesh()


## タイプを取得
func get_run_point_type() -> RunPointType:
	return _run_point_type
