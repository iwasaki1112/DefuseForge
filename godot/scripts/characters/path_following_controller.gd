...
func add_vision_point_to_extension(path_ratio: float, anchor: Vector3, target_point: Vector3) -> void:
	...
	if _has_extension:
		...
		if Debug.enabled: print("[PointDebug] PathFollowingCtrl.add_vision_to_ext: added to pending (count=%d)" % _extension_vision_points.size())
	else:
		# 延長パスに切り替わっている場合はチェッカーに追加
		_vision_checker.add_point(new_vp)
		if Debug.enabled: print("[PointDebug] PathFollowingCtrl.add_vision_to_ext: added to checker")
...
func set_extension_path(extension_path: Array[Vector3], markers: Dictionary, append_to_existing: bool = false) -> void:
	if extension_path.size() < 2:
		if Debug.enabled: print("[PointDebug] PathFollowingCtrl.set_extension_path: path too short (%d)" % extension_path.size())
		return

	if Debug.enabled: print("[PointDebug] PathFollowingCtrl.set_extension_path: path_len=%d, append=%s, has_ext=%s" % [
	...
	_has_extension = true
	if Debug.enabled: print("[PointDebug] PathFollowingCtrl.set_extension_path: done, vision=%d, wait=%d" % [
